...
from models.attnunet.utils import inconv, down_block
from models.attnunet.utils import get_block
from models.attnunet.modules import attention_up_block

class AttentionUNet(nn.Module):

    IMAGE_SIZE = (416, 624)
    def __init__(self, in_ch, num_classes, base_ch=64, block='BasicBlock', pool=True):
        super().__init__()
        ...
        self.outc = nn.Sequential(nn.Conv2d(base_ch, num_classes, kernel_size=1), nn.Sigmoid())
    ...
    @classmethod
    def build(cls, in_channels: int, num_classes: int):
        model = cls(in_ch=in_channels, num_classes=num_classes)
        return model
